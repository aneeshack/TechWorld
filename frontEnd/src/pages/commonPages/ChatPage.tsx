...
const ChatPage = () => {
  ...
  const handleEmojiClick = (emojiObject:EmojiClickData) => {
    setNewMessage((prev) => prev + emojiObject.emoji); // Append emoji to text
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      //         : msg
      //     ).concat(newMessage.sender === user?._id ? [] : [newMessage]) // Add if received
      //   );
  
      //   // Update latestMessages for the contact
      //   const contactId :string = newMessage.sender === user?._id ? newMessage.reciever : newMessage.sender;
      //   setLatestMessages((prev) => ({
      //     ...prev,
      //     [contactId]: newMessage, // Set server-confirmed message as latest
      //   }));
      // });
      socket.on("receiveMessage", (newMessage: IMessage) => {
        console.log("Received message from server:", newMessage);
        ...
        // const recieverId = newMessage.reciever; // Already string or undefined
      
        if (!senderId || !recieverId) {
          console.error("Missing sender or reciever ID:", newMessage);
          return; // Prevent setting invalid key
        }
            ...
            {loading && !selectedInstructor ? (
              ...
              <ul className="space-y-2">
                {instructors.map((contact) => {
                  ...
                  // console.log(`Rendering ${contact.userName}, latestMessage:`, latestMessage); // Debug
                  console.log(`Rendering ${contact.userName}, latestMessage:`, latestMessage, 'latestMessages:', latestMessages);
                  return (
                    <li
                      key={contact._id}
