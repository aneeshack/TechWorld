...
const ForumCard: React.FC<{ course: IEnrollment }> = ({ course }) => {
    return (
      <Link
        to={`/student/dashboard/forum/${course.courseId}`}
      ...
      >
        <div className="p-4 flex items-start space-x-4">
          ...
          <div className="flex-shrink-0 flex items-center space-x-2">
            <img
              src={course?.courseDetails?.thumbnail || 'https://via.placeholder.com/40?text=Course'}
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900 truncate">
                {course?.courseDetails?.title || 'Untitled Course'}
              </h3>
            ...
            {/* Action Button */}
            <button className="mt-3 inline-flex items-center px-3 py-1 bg-blue-600 text-white text-xs font-medium rounded-full hover:bg-blue-700 transition-colors">
              Join Discussion
...
const ForumsPage: React.FC = () => {
  ...
  <div className="bg-white rounded-lg shadow-md p-8 max-w-md w-full text-center">
    ...
    <Link
      to="/courseList"
      className="mt-6 inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-full hover:bg-blue-700 transition-colors"
    >
      Explore Courses
    </Link>
  </div>
